(() => {
  ...
  overlay.innerHTML = '<video class="itachi-video" src="assets/itachi/intro.mp4" playsinline preload="auto" aria-hidden="true"></video><button class="itachi-skip" type="button">Skip intro · Esc</button>';
  body.append(overlay);
  ...
  audio.addEventListener('error', () => { if(intro){finish(); message.textContent='The intro could not load. Itachi Mode is ready.';} });
  const art = new Image(); art.src = 'assets/itachi/shadow.png';
  ...
  function enable(){active=true;intro=true;previousOverflow=body.style.overflow;body.classList.add('itachi-mode');toggle.setAttribute('aria-pressed','true');toggle.disabled=true;sound.hidden=false;overlay.hidden=false;overlay.classList.remove('itachi-fade');overlay.dataset.scene='black';main.inert=true;body.style.overflow='hidden';skip.focus({preventScroll:true});
    ...
    audio.play().catch(()=>{if(!intro)return;audio.muted=true;audio.play().catch(()=>{if(intro)finish();});});
    later(()=>{if(intro){finish();message.textContent='Intro skipped because playback stalled.';}},30000);
  }
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden){audio.pause();cancelAnimationFrame(raf);raf=0;}else if(active){startFrames();if(intro)audio.play().catch(()=>finish());}});
